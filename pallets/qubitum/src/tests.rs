...
fn legacy_active_route_index_key(storage_name: &'static [u8], subnet_id: u16) -> Vec<u8> {
    legacy_twox_storage_map_key(storage_name, subnet_id)
}

fn legacy_twox_storage_map_key<K: Encode>(storage_name: &'static [u8], key: K) -> Vec<u8> {
    [
...
fn storage_keys_hide_linkable_ids_but_track_remaining_privacy_gaps() {
    new_test_ext().execute_with(|| {
        ...
        for (storage_name, storage_key) in [
            ("Subnets", Subnets::<Test>::hashed_key_for(subnet_id)),
            (
        ...
        let miner_key = Miners::<Test>::hashed_key_for(miner_id);
        assert!(
            !contains_subsequence(&miner_key, &miner_id.encode()),
            "Miners storage key exposes the raw miner id"
        );
        for (storage_name, storage_key) in [
            (
            ...
            assert!(
                !contains_subsequence(&storage_key, &miner_id.encode()),
                "{storage_name} storage key exposes the raw miner id"
            );
        }
        assert!(
            ...
            "miner locked bond storage key exposes the raw miner id"
        );
        assert!(
            !contains_subsequence(
        ...
        let validator_key = Validators::<Test>::hashed_key_for(validator_id);
        assert!(
            !contains_subsequence(&validator_key, &validator_id.encode()),
            "Validators storage key exposes the raw validator id"
        );
        for (storage_name, storage_key) in [
            (
            ...
            assert!(
                !contains_subsequence(&storage_key, &validator_id.encode()),
                "{storage_name} storage key exposes the raw validator id"
            );
        }
        assert!(
            !contains_subsequence(
            ...
            "validator locked stake storage key exposes the raw validator id"
        );
        assert!(
            !contains_subsequence(
...
#[test]
fn identity_signature_bundle_rejects_non_dilithium_commitments() {
    new_test_ext().execute_with(|| {
        register_active_miner_and_validator();
        ...
        let non_dilithium_miner_bundle = SignatureBundle {
            classical: None,
            post_quantum: Some(challenge_bound_signature(
        ...
        let non_dilithium_validator_bundle = SignatureBundle {
            classical: None,
            post_quantum: Some(challenge_bound_signature(
        ...
        assert_noop!(
            Qubitum::set_validator_identity_commitments(
                ...
                Some(commitment(70)),
                Some(commitment(71)),
                non_dilithium_validator_bundle,
            ),
            Error::<Test>::InvalidSignatureBundle
        );
        assert_eq!(
...
fn runtime_upgrade_migrates_subnet_owner_and_policy_to_commitments() {
    new_test_ext().execute_with(|| {
        ...
        let legacy_subnet_key = legacy_twox_storage_map_key(b"Subnets", 7_u16);
        sp_io::storage::set(&legacy_subnet_key, &legacy.encode());
        SubnetCount::<Test>::put(8);
        assert!(unhashed::get_raw(&legacy_subnet_key).is_some());
        assert!(Subnets::<Test>::get(7).is_none());
        assert!(
            !contains_subsequence(&Subnets::<Test>::hashed_key_for(7), &7_u16.encode()),
            "Subnets storage key exposes the raw subnet id"
        );
        StorageVersion::new(15).put::<crate::Pallet<Test>>();

        <Qubitum as Hooks<u64>>::on_runtime_upgrade();

        assert!(unhashed::get_raw(&legacy_subnet_key).is_none());
...
fn runtime_upgrade_migrates_registry_operators_to_commitments() {
    new_test_ext().execute_with(|| {
        ...
        let legacy_miner_key = legacy_twox_storage_map_key(b"Miners", 7_u64);
        let legacy_validator_key = legacy_twox_storage_map_key(b"Validators", 9_u64);
        sp_io::storage::set(&legacy_miner_key, &legacy_miner.encode());
        sp_io::storage::set(&legacy_validator_key, &legacy_validator.encode());
        MinerCount::<Test>::put(8);
        ...
        assert!(Miners::<Test>::get(7).is_none());
        assert!(Validators::<Test>::get(9).is_none());
        assert!(!contains_subsequence(
            &Miners::<Test>::hashed_key_for(7),
            &7_u64.encode()
        ));
        assert!(!contains_subsequence(
...
#[test]
fn runtime_upgrade_migrates_legacy_reversible_registry_record_keys() {
    new_test_ext().execute_with(|| {
        let miner = ChainMiner {
            id: 7,
            operator_commitment: commitment(22),
            subnet_id: 3,
            model_commitment: commitment(44),
            proof_system: ProofSystem::RiscZeroStark,
            bond_commitment: commitment(55),
            status: RegistryStatus::Active,
        };
        let validator = ChainValidator {
            id: 9,
            operator_commitment: commitment(33),
            subnet_id: 3,
            stake_commitment: commitment(66),
            status: RegistryStatus::Active,
        };

        let legacy_miner_key = legacy_twox_storage_map_key(b"Miners", 7_u64);
        let legacy_validator_key = legacy_twox_storage_map_key(b"Validators", 9_u64);
        sp_io::storage::set(&legacy_miner_key, &miner.encode());
        sp_io::storage::set(&legacy_validator_key, &validator.encode());
        MinerCount::<Test>::put(8);
        ValidatorCount::<Test>::put(10);
        assert!(unhashed::get_raw(&legacy_miner_key).is_some());
        assert!(unhashed::get_raw(&legacy_validator_key).is_some());
        ...
        StorageVersion::new(23).put::<crate::Pallet<Test>>();

        <Qubitum as Hooks<u64>>::on_runtime_upgrade();

        assert!(unhashed::get_raw(&legacy_miner_key).is_none());
        assert!(unhashed::get_raw(&legacy_validator_key).is_none());
        assert_eq!(Miners::<Test>::get(7), Some(miner));
...
#[test]
fn runtime_upgrade_migrates_legacy_reversible_capital_record_keys() {
    new_test_ext().execute_with(|| {
        register_active_miner_and_validator();
        let legacy_miner_bond_key = legacy_twox_storage_map_key(b"MinerLockedBond", 0_u64);
        let legacy_validator_stake_key =
            legacy_twox_storage_map_key(b"ValidatorLockedStake", 0_u64);
        MinerLockedBond::<Test>::remove(0);
        ValidatorLockedStake::<Test>::remove(0);
        sp_io::storage::set(&legacy_miner_bond_key, &MIN_MINER_BOND.encode());
        sp_io::storage::set(&legacy_validator_stake_key, &MIN_MINER_BOND.encode());
        assert!(unhashed::get_raw(&legacy_miner_bond_key).is_some());
        assert!(unhashed::get_raw(&legacy_validator_stake_key).is_some());
        assert_eq!(MinerLockedBond::<Test>::get(0), None);
        assert_eq!(ValidatorLockedStake::<Test>::get(0), None);
        assert!(
            !contains_subsequence(&MinerLockedBond::<Test>::hashed_key_for(0), &0_u64.encode()),
            "miner locked bond storage key exposes the raw miner id"
        );
        assert!(
            !contains_subsequence(
                &ValidatorLockedStake::<Test>::hashed_key_for(0),
                &0_u64.encode()
            ),
            "validator locked stake storage key exposes the raw validator id"
        );
        StorageVersion::new(19).put::<crate::Pallet<Test>>();

        <Qubitum as Hooks<u64>>::on_runtime_upgrade();

        assert!(unhashed::get_raw(&legacy_miner_bond_key).is_none());
...
#[test]
fn runtime_upgrade_migrates_legacy_reversible_identity_record_keys() {
    new_test_ext().execute_with(|| {
        register_active_miner_and_validator();

        let miner_commitments = MinerIdentityCommitments::<Test>::get(0).unwrap();
        let miner_bundle = MinerIdentitySignatureBundles::<Test>::get(0).unwrap();
        let miner_challenge = MinerIdentitySignatureChallenges::<Test>::get(0).unwrap();
        let validator_commitments = ValidatorIdentityCommitments::<Test>::get(0).unwrap();
        let validator_bundle = ValidatorIdentitySignatureBundles::<Test>::get(0).unwrap();
        let validator_challenge = ValidatorIdentitySignatureChallenges::<Test>::get(0).unwrap();

        let legacy_miner_commitments_key =
            legacy_twox_storage_map_key(b"MinerIdentityCommitments", 0_u64);
        let legacy_miner_bundle_key =
            legacy_twox_storage_map_key(b"MinerIdentitySignatureBundles", 0_u64);
        let legacy_miner_challenge_key =
            legacy_twox_storage_map_key(b"MinerIdentitySignatureChallenges", 0_u64);
        let legacy_validator_commitments_key =
            legacy_twox_storage_map_key(b"ValidatorIdentityCommitments", 0_u64);
        let legacy_validator_bundle_key =
            legacy_twox_storage_map_key(b"ValidatorIdentitySignatureBundles", 0_u64);
        let legacy_validator_challenge_key =
            legacy_twox_storage_map_key(b"ValidatorIdentitySignatureChallenges", 0_u64);

        MinerIdentityCommitments::<Test>::remove(0);
        MinerIdentitySignatureBundles::<Test>::remove(0);
        MinerIdentitySignatureChallenges::<Test>::remove(0);
        ValidatorIdentityCommitments::<Test>::remove(0);
        ValidatorIdentitySignatureBundles::<Test>::remove(0);
        ValidatorIdentitySignatureChallenges::<Test>::remove(0);

        sp_io::storage::set(&legacy_miner_commitments_key, &miner_commitments.encode());
        sp_io::storage::set(&legacy_miner_bundle_key, &miner_bundle.encode());
        sp_io::storage::set(&legacy_miner_challenge_key, &miner_challenge.encode());
        sp_io::storage::set(
            &legacy_validator_commitments_key,
            &validator_commitments.encode(),
        );
        sp_io::storage::set(&legacy_validator_bundle_key, &validator_bundle.encode());
        sp_io::storage::set(
            &legacy_validator_challenge_key,
            &validator_challenge.encode(),
        );

        for legacy_key in [
            &legacy_miner_commitments_key,
            &legacy_miner_bundle_key,
            &legacy_miner_challenge_key,
            &legacy_validator_commitments_key,
            &legacy_validator_bundle_key,
            &legacy_validator_challenge_key,
        ] {
            assert!(unhashed::get_raw(legacy_key).is_some());
        }
        assert_eq!(MinerIdentityCommitments::<Test>::get(0), None);
        assert_eq!(MinerIdentitySignatureBundles::<Test>::get(0), None);
        assert_eq!(MinerIdentitySignatureChallenges::<Test>::get(0), None);
        assert_eq!(ValidatorIdentityCommitments::<Test>::get(0), None);
        assert_eq!(ValidatorIdentitySignatureBundles::<Test>::get(0), None);
        assert_eq!(ValidatorIdentitySignatureChallenges::<Test>::get(0), None);
        for (storage_name, storage_key) in [
            (
                "MinerIdentityCommitments",
                MinerIdentityCommitments::<Test>::hashed_key_for(0),
            ),
            (
                "MinerIdentitySignatureBundles",
                MinerIdentitySignatureBundles::<Test>::hashed_key_for(0),
            ),
            (
                "MinerIdentitySignatureChallenges",
                MinerIdentitySignatureChallenges::<Test>::hashed_key_for(0),
            ),
            (
                "ValidatorIdentityCommitments",
                ValidatorIdentityCommitments::<Test>::hashed_key_for(0),
            ),
            (
                "ValidatorIdentitySignatureBundles",
                ValidatorIdentitySignatureBundles::<Test>::hashed_key_for(0),
            ),
            (
                "ValidatorIdentitySignatureChallenges",
                ValidatorIdentitySignatureChallenges::<Test>::hashed_key_for(0),
            ),
        ] {
            assert!(
                !contains_subsequence(&storage_key, &0_u64.encode()),
                "{storage_name} storage key exposes the raw participant id"
            );
        }
        StorageVersion::new(20).put::<crate::Pallet<Test>>();
        ...
        assert_eq!(
            MinerIdentitySignatureChallenges::<Test>::get(0),
            Some(miner_challenge)
        );
        assert_eq!(
            ValidatorIdentityCommitments::<Test>::get(0),
            Some(validator_commitments)
        );
        assert_eq!(
            ValidatorIdentitySignatureBundles::<Test>::get(0),
            Some(validator_bundle)
        );
        assert_eq!(
            ValidatorIdentitySignatureChallenges::<Test>::get(0),
...
fn runtime_upgrade_identity_missing_records_do_not_overflow_attested_routes() {
    new_test_ext().execute_with(|| {
        ...
        MinerCount::<Test>::put(attested_miner_id.saturating_add(1));
        ValidatorCount::<Test>::put(attested_validator_id.saturating_add(1));
        ActiveMinersBySubnet::<Test>::remove(0);
        ActiveValidatorsBySubnet::<Test>::remove(0);
        StorageVersion::new(17).put::<crate::Pallet<Test>>();
...
fn runtime_upgrade_demotes_overflow_routing_index_participants_and_reports_health() {
    new_test_ext().execute_with(|| {
        ...
        MinerCount::<Test>::put(pending_miner_id.saturating_add(1));
        ValidatorCount::<Test>::put(pending_validator_id.saturating_add(1));
        ActiveMinersBySubnet::<Test>::try_mutate(0, |ids| {
...
fn runtime_upgrade_reports_missing_legacy_capital_records() {
    new_test_ext().execute_with(|| {
        ...
        MinerCount::<Test>::put(2);
        ValidatorCount::<Test>::put(2);
        StorageVersion::new(17).put::<crate::Pallet<Test>>();
